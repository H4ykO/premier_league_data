...
def sample_standings():
    return pd.DataFrame([
        {'position': 1, 'team': 'Arsenal', 'season': 2024, 'played': 10, 'won': 7,
         'drawn': 2, 'lost': 1, 'goals_for': 20, 'goals_against': 8, 'points': 23},
        {'position': 2, 'team': 'Chelsea', 'season': 2024, 'played': 10, 'won': 6,
         'drawn': 1, 'lost': 3, 'goals_for': 15, 'goals_against': 10, 'points': 19},
    ])
...
def sample_matches():
    return pd.DataFrame([
        {'match_id': 1, 'season': 2024, 'date': '2024-01-01', 'home_team': 'Arsenal',
         'away_team': 'Chelsea',   'home_score': 2, 'away_score': 1, 'status': 'FINISHED'},
        {'match_id': 2, 'season': 2024, 'date': '2024-01-08', 'home_team': 'Chelsea',
         'away_team': 'Liverpool', 'home_score': 0, 'away_score': 0, 'status': 'FINISHED'},
        {'match_id': 3, 'season': 2024, 'date': '2024-01-15', 'home_team': 'Arsenal',
         'away_team': 'Liverpool', 'home_score': 1, 'away_score': 3, 'status': 'SCHEDULED'},
    ])
...
def test_transform_standings_preserva_coluna_season(sample_standings):
    result = transform_standings(sample_standings)
...
def test_transform_matches_remove_coluna_status(sample_matches):
    result = transform_matches(sample_matches)
...
def test_transform_matches_preserva_coluna_season(sample_matches):
    result = transform_matches(sample_matches)
    assert 'season' in result.columns
    assert result.iloc[0]['season'] == 2024
